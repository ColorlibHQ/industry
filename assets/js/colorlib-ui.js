/*! ColorlibUI 3.0.0 — core, owl, magnific, ajaxchimp, datepicker, superfish, owl markup 2.0. Built for this theme from core + the modules it uses. */
/**
...
/* ColorlibUI module: superfish — replaces Superfish 1.7.9 (hover dropdown menus).
 * Adds the same classes (sf-js-enabled, sf-arrows on the menu, sf-with-ul on
 * parent links, sfHover on open items), leaves the same inline styles on the
 * submenus (display: none/block, opacity while fading) and keeps the plugin's
 * timing: open on mouseenter or keyboard focus, close `delay` ms after the
 * pointer or focus leaves, siblings close when another item opens, and a first
 * tap on a touch screen opens the submenu instead of following the link.
 * Callbacks get the submenu Element as `this` instead of a jQuery object. */
(function () {
  'use strict';
  var UI = window.ColorlibUI;
  if (!UI) return;
  ...
  var DEFAULTS = {
    ...
    delay: 800, animation: { opacity: 'show' }, animationOut: { opacity: 'hide' }, speed: 'normal',
    speedOut: 'fast', cssArrows: true, disableHI: false, onInit: noop, onBeforeShow: noop, onShow: noop,
    onBeforeHide: noop, onHide: noop, onIdle: noop, onDestroy: noop, onHandleTouch: noop
  };
  var SPEEDS = { slow: 600, fast: 200 };
  var IOS = /^(?![\w\W]*Windows Phone)[\w\W]*(iPhone|iPad|iPod)/i.test(navigator.userAgent);

  function duration(v) { return typeof v === 'number' ? v : (SPEEDS[v] || 400); }
  function kids(el, sel) {
    return Array.prototype.filter.call(el.children, function (c) { return c.matches(sel); });
  }
  function isHidden(el) { return getComputedStyle(el).display === 'none'; }
  /** jQuery's show(): the remembered display, else the stylesheet's, else block. */
  function reveal(el) {
    if (el._clOldDisplay) { el.style.display = el._clOldDisplay; return; }
    el.style.display = '';
    if (isHidden(el)) el.style.display = 'block';
  }

  /*
   * jQuery .stop(true, true).animate({opacity: 'show', height: 'hide', ...}, ms):
   * only 'show'/'hide'/'toggle' values are used by these menus. Inline values
   * are cleared at the end, leaving `display: block` (shown) or `display: none`.
   */
  function animate(el, props, ms, done) {
    if (el._clFx) el._clFx();
    var hidden = isHidden(el), keys = [], show = null, k;
    for (k in props) {
      var v = props[k] === 'toggle' ? (hidden ? 'show' : 'hide') : props[k];
      if ((v === 'show' && hidden) || (v === 'hide' && !hidden)) { keys.push(k); show = v === 'show'; }
    }
    if (!keys.length) { setTimeout(done, 0); return; }
    if (show) reveal(el);
    var cs = getComputedStyle(el), full = {};
    keys.forEach(function (p) { full[p] = parseFloat(cs[p]) || 0; });
    var box = keys.indexOf('height') > -1 || keys.indexOf('width') > -1;
    var frame, finished = false, start = null;
    function set(e) {
      keys.forEach(function (p) {
        var val = full[p] * (show ? e : 1 - e);
        el.style[p] = p === 'opacity' ? String(val) : val + 'px';
      });
    }
    function finish() {
      if (finished) return;
      ...
      el._clFx = null;
      if (!show) {
        // jQuery remembers the display it overwrites and restores it on the next show.
        if (el.style.display !== 'none') el._clOldDisplay = el.style.display;
        el.style.display = 'none';
      }
      keys.forEach(function (p) { el.style[p] = ''; });
      if (box) el.style.overflow = '';
      done();
    }
    if (!ms || UI.reducedMotion) { finish(); return; }
    if (box) el.style.overflow = 'hidden';
    set(0);
    el._clFx = finish;
    frame = requestAnimationFrame(function step(now) {
      if (start === null) start = now;
      var p = Math.min((now - start) / ms, 1);
      set(0.5 - Math.cos(p * Math.PI) / 2);
  ...
  function Menu(menu, options) {
    var self = this;
    var o = self.o = UI.extend({}, DEFAULTS, options);
    self.menu = menu;
    self.timer = null;
    self.retainPath = false;

    // Breadcrumb path (pathClass): stays open while the menu is idle.
    self.path = Array.prototype.slice.call(menu.querySelectorAll('li.' + o.pathClass), 0, o.pathLevels);
    self.path.forEach(function (li) {
      li.classList.add(o.hoverClass, 'sf-breadcrumb');
      if (kids(li, o.popUpSelector).length) li.classList.remove(o.pathClass);
    });

    menu.classList.add('sf-js-enabled');
    if (o.cssArrows) menu.classList.add('sf-arrows');
    var parents = [];
    Array.prototype.forEach.call(menu.querySelectorAll(o.popUpSelector), function (pop) {
      var li = pop.parentNode;
      if (li && li.tagName === 'LI' && parents.indexOf(li) < 0) parents.push(li);
    });
    self.parents = parents;
    parents.forEach(function (li) { kids(li, 'a').forEach(function (a) { a.classList.add('sf-with-ul'); }); });
    // Superfish flips touch-action so vertical panning still scrolls the page on touch screens.
    var ta = getComputedStyle(menu).touchAction;
    menu.style.touchAction = ta === 'pan-y' ? 'auto' : 'pan-y';

    Array.prototype.forEach.call(menu.querySelectorAll('li'), function (li) {
      if (!li.querySelector(o.popUpSelector)) return;
      li.addEventListener('mouseenter', function () { self.over(li); });
      li.addEventListener('mouseleave', function () { self.out(li); });
    });
    // Delegated focusin/focusout ran once per li between the target and the menu.
    function walk(e, fn) {
      for (var n = e.target; n && n !== menu; n = n.parentNode) if (n.tagName === 'LI') fn.call(self, n);
    }
    menu.addEventListener('focusin', function (e) { walk(e, self.over); });
    menu.addEventListener('focusout', function (e) { walk(e, self.out); });
    function touch(e) {
      var a = e.target.closest ? e.target.closest('a') : null;
      if (!a || !menu.contains(a)) return;
      var pops = Array.prototype.filter.call(a.parentNode.children, function (c) { return c !== a && c.matches(o.popUpSelector); });
      if (o.onHandleTouch.call(pops[0]) === false) return;
      if (pops.length && isHidden(pops[0])) {
        // First tap opens the submenu; the click that follows must not navigate.
        a.addEventListener('click', function block(ev) {
          a.removeEventListener('click', block);
          ev.preventDefault();
          ev.stopPropagation();
        });
        if (e.type === 'pointerdown') a.focus(); else self.over(a.parentNode);
      }
    }
    menu.addEventListener(window.PointerEvent ? 'pointerdown' : 'MSPointerDown', touch);
    if (!IOS) menu.addEventListener('touchend', touch);

    parents.forEach(function (li) { if (!li.classList.contains('sf-breadcrumb')) self.hide(li, true); });
    o.onInit.call(menu);
  }

  Menu.prototype.show = function (li) {
    var o = this.o;
    li.classList.add(o.hoverClass);
    var pops = kids(li, o.popUpSelector);
    if (o.onBeforeShow.call(pops[0]) === false) return;
    pops.forEach(function (pop) { animate(pop, o.animation, duration(o.speed), function () { o.onShow.call(pop); }); });
  };

  Menu.prototype.hide = function (li, immediate) {
    var o = this.o, self = this;
    var lis = Array.prototype.slice.call(li.querySelectorAll('li.' + o.hoverClass));
    lis.push(li);
    var pops = [];
    lis.forEach(function (x) {
      if (self.retainPath && self.path.indexOf(x) > -1) return;
      x.classList.remove(o.hoverClass);
      pops = pops.concat(kids(x, o.popUpSelector));
    });
    self.retainPath = false;
    if (o.onBeforeHide.call(pops[0]) === false) return;
    pops.forEach(function (pop) {
      // .show() first, so a submenu hidden by the theme CSS still ends with an inline display: none.
      if (immediate) reveal(pop);
      animate(pop, o.animationOut, immediate ? 0 : duration(o.speedOut), function () { o.onHide.call(pop); });
    });
  };

  /** Open li now and close its siblings. */
  Menu.prototype.over = function (li) {
    var self = this;
    clearTimeout(self.timer);
    Array.prototype.forEach.call(li.parentNode.children, function (sib) { if (sib !== li) self.hide(sib); });
    self.show(li);
  };

  /** Close li after the delay (one pending close per menu, as in Superfish). */
  Menu.prototype.out = function (li) {
    var self = this;
    function close() {
      self.retainPath = self.path.indexOf(li) > -1;
      self.hide(li);
      var p = li.parentNode, openAncestor = false;
      for (; p && p !== document; p = p.parentNode) if (p.classList && p.classList.contains(self.o.hoverClass)) openAncestor = true;
      if (!openAncestor) {
        self.o.onIdle.call(self.menu);
        if (self.path.length) self.path.forEach(function (x) { self.over(x); });
      }
    }
    if (IOS) { close(); return; }
    clearTimeout(self.timer);
    self.timer = setTimeout(close, self.o.delay);
  };

  /** $(menu).superfish(options) → ColorlibUI.superfish(menu, options). */
  UI.superfish = function (target, options) {
    var out = [];
    UI.toElements(target).forEach(function (menu) {
      if (!menu._clSuperfish) {
        menu._clSuperfish = new Menu(menu, options);
        menu.setAttribute('data-cl-superfish', '1');
      }
      out.push(menu._clSuperfish);
    });
    return out;
  };
}());

window.ColorlibUI.owl.defaults.markup = '2.0';
