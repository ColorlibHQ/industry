...
(function () {
	...
	function run() {
		...
		var notice = window.industryElementorNotice;
		if ( ! UI || ! notice ) {
			return;
		}
		...
		UI.toElements( '.industry-elementor-notice-buttons > a' ).forEach(
			function ( button ) {
						...
						UI.request(
							...
							function () {
								...
								if ( reply === 'yes' ) {
									...
									UI.fade( '.industry-disable-elementor-styling', 'out', 500, function () { this.remove(); } );
								}
							}
						);
					}
